"""LGBTQ+ bars & restaurants — worldwide venue GeoJSON from OpenStreetMap.

Queries the OpenStreetMap Overpass API for food/drink venues (bar, pub,
nightclub, restaurant, cafe, fast_food, biergarten) tagged as LGBTQ-oriented or
LGBTQ-welcoming (``lgbtq=*`` other than ``no``, or the older ``gay=yes``), and
caches them as a single GeoJSON FeatureCollection under::

    cache/save-earth/lgbtq/lgbtq_venues.geojson + .meta.json

Each cached Feature keeps **all** of the element's OSM tags verbatim as its
``properties`` (so the map popup table shows everything OSM knows — name,
amenity, lgbtq, opening_hours, website, …), plus derived ``osm_type``/``osm_id``
(provenance), ``osm_url`` (deep link), and ``feature_kind`` (the amenity value).

Coverage is OSM-community-driven and varies by region (best in Europe / North
America). Pass ``use_mock=True`` for a small offline set when Overpass is
unavailable.
"""
...
logger = logging.getLogger("save-earth.lgbtq")

NAMESPACE = "save-earth"
CACHE_TYPE = "lgbtq"
RELATIVE_PATH = "lgbtq_venues.geojson"
...
_AMENITIES = "^(bar|pub|nightclub|restaurant|cafe|fast_food|biergarten)$"
# LGBTQ-oriented/welcoming food+drink venues. ``lgbtq=no`` (explicitly NOT
# LGBTQ) is dropped in _to_feature.
OVERPASS_QUERY = (
    "[out:json][timeout:240];"
    "("
    f'nwr["lgbtq"]["amenity"~"{_AMENITIES}"];'
    f'nwr["gay"="yes"]["amenity"~"{_AMENITIES}"];'
    ");"
    "out center tags;"
)
...
def download(
    *,
    ...
    """Fetch worldwide LGBTQ+ bars & restaurants and cache them as GeoJSON."""
        ...
        if not force:
            ...
            if side and sidecar.exists_and_valid(NAMESPACE, CACHE_TYPE, RELATIVE_PATH, s):
                ...
                if age is None or age < max_age_hours:
                    logger.info("lgbtq cache hit (%.1fh old)", age or -1.0)
...
def _fetch_overpass() -> tuple[list[dict[str, Any]], str]:
    ...
    for endpoint in OVERPASS_ENDPOINTS:
        ...
        logger.info("Overpass %s → %d lgbtq venues", endpoint, len(features))
...
def _to_feature(el: dict[str, Any]) -> dict[str, Any] | None:
    tags = el.get("tags") or {}
    if str(tags.get("lgbtq", "")).lower() == "no":  # explicitly NOT LGBTQ → skip
        return None
    ...
    props["feature_kind"] = tags.get("amenity", "venue")
...
def _mock_features() -> list[dict[str, Any]]:
    base = [
        {"name": "Stonewall Inn", "amenity": "bar", "lgbtq": "primary",
         "country": "US", "lat": 40.7340, "lon": -74.0021},
        {"name": "The Eagle", "amenity": "bar", "lgbtq": "primary",
         "country": "US", "lat": 40.7456, "lon": -74.0072},
        {"name": "Heaven", "amenity": "nightclub", "lgbtq": "primary",
         "country": "GB", "lat": 51.5085, "lon": -0.1233},
        {"name": "SchwuZ", "amenity": "nightclub", "lgbtq": "primary",
         "country": "DE", "lat": 52.4823, "lon": 13.4317},
        {"name": "Le Cox", "amenity": "bar", "lgbtq": "primary",
         "country": "FR", "lat": 48.8601, "lon": 2.3553},
    ]
    features: list[dict[str, Any]] = []
    for i, row in enumerate(base):
        tags = {k: v for k, v in row.items() if k not in ("lat", "lon")}
        tags["feature_kind"] = row["amenity"]
        tags["osm_type"] = "node"
        tags["osm_id"] = 300000 + i
        features.append({
            "type": "Feature",
